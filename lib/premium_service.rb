    ...
    inactive_devices = customer.insured_devices.select { |id| id.imei.nil? && (!id.quote.policy.nil?) }
      ...
      if !inactive_devices.empty?
        ...
        if policy.status == "Active"
          ...
          sms_gateway.send phone_number, "You have successfully covered your device, value #{insured_value_str}. Orient Mobile policy #{policy.policy_number} valid till #{policy.expiry.to_s(:simple)}. Policy details: www.korient.co.ke/OMB/T&amp;C"
