    ...
    case step
      when :customer_id
        if @customer.nil?
          ...
          devs = InsuredDevice.joins('inner join quotes on quotes.insured_device_id = insured_devices.id').where('insured_devices.customer_id = ? and quotes.premium_type != ?', @customer.id, '')
